...
DirectSampler <- function(X, coeff_1, sig2_1, lbd_1, coeff_2, sig2_2, lbd_2,
  ...
  method = "normal", Y, parallel = FALSE, ncores = 2L, verbose = FALSE)
{
  ...
  if (Mixture) {
    ...
    DS1 <- DirectSamplerMain(X = X, coeff = coeff_1, sig2 = sig2_1,
                         ...
                         method = method, Y=Y, parallel = parallel, ncores = ncores, verbose = verbose)
...
DirectSamplerMain <- function(X, coeff, mu, sig2, lbd, weights = rep(1, max(group)),
 group = 1:ncol(X), niter = 2000, method = "normal", Y, parallel = FALSE,
