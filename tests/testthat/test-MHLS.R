...
test_that("Low dimensional setting under lasso setting", {
  ...
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 1,
                    ...
                    type = "coeff", verbose = FALSE), NA)
  expect_error(MHLS(X = X, pointEstimate = rep(0, n), sig2 = 1, lbd = 1,
                    B0 = B0, S0 = S0, niter = 50, burnin = 0,
                    type = "mu", verbose = FALSE), NA)
  expect_error(MHLS(X = X, pointEstimate = rep(0, n), sig2 = 1, lbd = 1,
                    B0 = B0, niter = 50, burnin = 0,
                    type = "mu", verbose = FALSE), NA)
  expect_error(MHLS(X = X, pointEstimate = rep(0, n), sig2 = 1, lbd = 1,
  ...
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 1,
                    ...
                    type = "coeff", verbose = FALSE)
               , "Invalid active")
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 1,
                    B0 = B0, S0 = S0, A = c(1,1:p), niter = 50, burnin = 0,
                    type = "coeff", verbose = FALSE)
               , "Invalid active")
  expect_error(MHLS(X = X, pointEstimate = rep(0, p-1), sig2 = 1, lbd = 1,
                    B0 = B0, S0 = S0, niter = 50, burnin = 0,
                    type = "coeff", verbose = FALSE)
               , "length")
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 1,
  ...
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = -1, lbd = 1,
                    ...
                    type = "coeff", verbose = FALSE)
               , "sig2 and/or lbd")
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 0,
                    B0 = B0, S0 = S0, niter = 50, burnin = 0,
                    type = "coeff", verbose = FALSE)
               , "sig2 and/or lbd")
  expect_error(MHLS(X = X, pointEstimate = rep(0, p), sig2 = 1, lbd = 1,
...
LassoResult <- Lasso.MHLS(X = X, Y = Y, group = group, lbd = lbd, weights = weights)
B0 <- LassoResult$B0
S0 <- LassoResult$S0
