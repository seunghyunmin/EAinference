...
test_that("group argument",{
  ...
  expect_error(PBsampler(X = x, PE_1 = rep(0, p), sig2_1 = 1, lbd_1 = .5,
               ...
               , NA)
  expect_error(PBsampler(X = x, PE_1 = rep(0, p), sig2_1 = 1, lbd_1 = .5,
                             weights = Weights1, group = Group1, niter = Niter, type = "grlasso", PEtype = "coeff", parallel = FALSE)
  , NA)
  expect_error(PBsampler(X = x, PE_1 = rep(0, p), sig2_1 = 1, lbd_1 = .5,
                             weights = Weights2, group = Group2, niter = Niter, type = "grlasso", PEtype = "coeff", parallel = FALSE)
  , "group must have a same length with the number of X columns")
  expect_error(PBsampler(X = x, PE_1 = rep(0, p), sig2_1 = 1, lbd_1 = .5,
                             weights = Weights3, group = Group3, niter = Niter, type = "grlasso", PEtype = "coeff", parallel = FALSE)
  , "group index has to be a consecutive integer starting from 1"
  )
  expect_error(PBsampler(X = x, PE_1 = rep(0, p), sig2_1 = 1, lbd_1 = .5,
                             weights = Weights4, group = Group4, niter = Niter, type = "grlasso", PEtype = "coeff", parallel = FALSE)
  , "group index has to be a consecutive integer starting from 1"
  )
})
...
test_that("confidence interval", {
  ...
  X <- matrix(rnorm(n*p), n)
  object <- PBsampler(X = X,PE_1 = c(1,1,rep(0,p-2)),sig2_1 = 1,lbd_1 = .5,niter=100,type="lasso",parallel=parallel,ncores=ncores)
  expect_error(PB.CI(object = object, alpha = .05, method = "debias",parallel=parallel,ncores=ncores)
               , NA)
  expect_error(PB.CI(object = object, alpha = .05, method = "none",parallel=parallel,ncores=ncores)
               , NA)
  object <- PBsampler(X = X,PE_1 = c(1,1,rep(0,p-2)),sig2_1 = 1,lbd_1 = 1,niter=100,type="slasso",parallel=parallel,ncores=ncores)
  expect_error(PB.CI(object = object, alpha = .05, method = "debias",parallel=parallel,ncores=ncores)
               , NA)
  expect_error(PB.CI(object = object, alpha = .05, method = "none")
               , NA)
  # object <- PBsampler(X = X,PE_1 = c(1,1,rep(0,p-2)),sig2_1 = 1,lbd_1 = .5,group=Group,niter=100,type="grlasso",parallel=parallel,ncores=ncores)
  # expect_error(PB.CI(object = object, alpha = .05, method = "debias",parallel=parallel,ncores=ncores)
  #              , NA)
  # expect_error(PB.CI(object = object, alpha = .05, method = "none")
  #              , NA)
  # object <- PBsampler(X = X,PE_1 = c(1,1,rep(0,p-2)),sig2_1 = 1,lbd_1 = 1,group=Group,niter=100,type="sgrlasso",parallel=parallel,ncores=ncores)
  # expect_error(PB.CI(object = object, alpha = .05, method = "debias",parallel=parallel,ncores=ncores)
  #              , NA)
  # expect_error(PB.CI(object = object, alpha = .05, method = "none")
  #              , NA)
